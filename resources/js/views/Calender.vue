      ...
      <v-card-text>
        <div class="event-content">
          ...
          >
            ...
            <option value="Payé" data-class="green-event">🟢 Payé</option>
            <option value="En-cours" data-class="yellow-event">
              🟡 en-cours
            </option>
            <option value="Annulé" data-class="red-event">🔴 Annulé</option>
          </select>
        </div>
        ...
        <v-text-field
          label="Prix"
          v-model="this.selectedEvent.price"
          outlined
...
export default {
  ...
  computed: {
    ...
    sports() {
      return this.splitTypes ? this.splitTypes.map((split) => split.type) : [];
    },
  },
  watch: {
    sports(newSports) {
  ...
  methods: {
    ...
    updateSplits() {
      ...
      const selectedSplitType = this.splitTypes.find(
        (splitType) =>
          splitType.type.toLowerCase() === this.selectedSport.toLowerCase()
      );
      ...
      if (selectedSplitType) {
        ...
        this.splitDays = selectedSplitType.terrains.map((terrain, index) => ({
          id: terrain.id,
          label: terrain.label,
    ...
    createEventInSplit(event) {
      if (!event.hasOwnProperty("split")) {
        alert("Veuillez configurer votre calendrier dans la page de configuration.");
        return;
      }
      // if (event.hasOwnProperty("split")) {
      ...
      if (!this.checkForCreationOverlapping(event)) {
        if (event.split) {
          ...
          const splitType = this.splitTypes.find((splitType) =>
            event.split.toLowerCase().includes(splitType.type.toLowerCase())
          );
          ...
          const prices = matchingTerrain ? matchingTerrain.prices : null;
          ...
          price = this.getPriceForTimeInterval(newDate, duration, prices) || 0;
